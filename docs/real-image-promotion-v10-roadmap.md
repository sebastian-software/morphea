...
current accepted region plan is checked in at
`docs/real-images/reviews/current-region-decision-plan.json`: it accepts the
transparent Terminaro and checked-in opaque generated-illustration gold-circle
shape-class and visual-fidelity regions via `reviewed_region_ids`, while the
UI radio case remains deferred.
